...
public class RStep{ //n = number, p = pattern, m = master, h = horizontal
    ...
    fun void init(int r, int c, int hl, int np, int ns, string s[], int nPort, int oPort, UGen lf, UGen rt){
        ...
        theCue => dac.chan(qLeft);
        theCue => dac.chan(qRight);
        mPan.left => lf;
        mPan.right => rt;
        int nstepCCs[nSteps];
    ...
    fun void play(){
        while(theClock.step => now){
            if(seqs[cPat][theClock.stepCount() % pLen]){
                if(theMute){ 
                    0 => mPan.gain;
                    trigger(cSound);
                }
            ...
            if(focus){
                updateGrid();
                if(!seqs[cPat][(theClock.stepCount()-1 +pLen) % pLen]){ 
                    midiOut(0x90, stepCCs[(theClock.stepCount()-1) % pLen], padOffClr);
                }
                if(seqs[cPat][theClock.stepCount() % pLen]){ 
                    midiOut(0x90, stepCCs[theClock.stepCount() % pLen], hiClr);
                }
    ...
    fun void setCurSound(float s){
        if(s>=0 & s<=1){
            Math.round(s*nSounds) $ int=>int intS;
            if(intS != cSound) intS => cSound;
        }
    }
