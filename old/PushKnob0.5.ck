...
public class PushKnob{
    ...
    fun void displayUpdate(){
        ...
        if(inFocus){
            if(displayCursor)makeCursor(cursorX,cursorY,2);
            if(displayValue)pD.setSubsegment(valX,valY,dValue);
    ...
    fun void clearValue(){
        pD.setSubsegment(valX,valY,"");
        if(selfUpdate)pD.updateDisplay();
    }
    
    fun void clearCursor(){
        pD.setSubsegment(cursorX,cursorY,"");
        if(selfUpdate)pD.updateDisplay();
    }
    
    fun void clearLabel(){
        pD.setSubsegment(labelX,labelY,"");
        if(selfUpdate)pD.updateDisplay();
    }
    ...
    fun void makeCursor(int c,int r,int style){
        ...
        if(style==0){
            ...
            pD.setSubsegment(c,r,cursorThing);
            if(selfUpdate)pD.updateDisplay();
        }else if(style==3){
