...
public class PushKnob{
    ...
    fun void incListen(string s[]){
        while(increment=>now){
            incVal+=>ticks;
            if(ticks>quant){
                quant-=>ticks;
                if(!wrap){if(sNum<(sList.cap()-1))1+=>sNum;}
                else(sNum+1)%sList.cap()=>sNum;
                text(sList[sNum]);
    ...
    fun void decListen(string s[]){
        while(decrement=>now){
            incVal+=>ticks;
            if(ticks<0){
                quant+=>ticks;
                if(!wrap){if(sNum>0)sNum-1=>sNum;}
                else(sNum-1+sList.cap())%sList.cap()=>sNum;
                text(sList[sNum]);
                pos(sNum/(sList.cap()-1.0));
                p.updateDisplay();
            }
