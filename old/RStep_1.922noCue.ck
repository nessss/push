...
public class RStep{ //n = number, p = pattern, m = master, h = horizontal, q = cue
    ...
    fun void init(int r, int c, int hl, int np, int ns, string s[], int nPort, int oPort, int outL, int outR, int qL, int qR){
        ...
        0 => muted, cued, cursor, cPat, cSound, thePan;
        0.5 => mGain, qGain;
        1 => focused, theRate;
        //Colors
        9 => int onClr; 
        ...
        for(0 => int i; i<nSounds; i++) sounds[i] => tempPans[i];
        tempPans @=> pans;
        for(0 => int i; i<nSounds; i++){ 
            pans[i] => mBus;
            pans[i] => qBus;
        }
        mBus.left => dac.chan(outL);  //Master out
        mBus.right => dac.chan(outR);
        qBus.left => blackhole;        //Cue out
        qBus.right => blackhole;
    ...
    fun int numPats(){ return tempPats; }
