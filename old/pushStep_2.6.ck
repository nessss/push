...
spork~knobValue(mGainKnob,mBus);
...
fun void unfocusPages(int pg){ //for single knobs that take whole display focus
    if(pg==0){ //master gain
        for(0=>int i;i<nDrums;i++){ //unfocus all knobs
            for(0=>int j; j<nPages; j++){
                for(0 => int l; l<4; l++){
                    knob[i][j][l].focus(0);
                }
...
fun void printMasterGain(PushKnob pK){
    while(pK.touchOn=>now){ 
        
    }
}

fun void hideMasterGain(PushKnob pK){
...
fun void knobValue(PushKnob pK, Pan2 p){ //for master gain knob
    while(pK.moved=>now){
        if(pK.value() >= 0 & pK.value() <=1){
            pK.value() => p.gain;
        }
    }
}
